...
Test.@testset "RayCaster.jl" begin
    ...
    Test.@testset "Discrete world" begin
        I = Int
        cells_per_tile_along_axis = convert(I, 256)

        i_ray_start_cell = convert(I, height_obstacle_tile_map * cells_per_tile_along_axis ÷ 2 + 1)
        j_ray_start_cell = convert(I, width_obstacle_tile_map * cells_per_tile_along_axis ÷ 2 + 1)

        Test.@testset "delta_i = 1, delta_j = 0" begin
            i_ray_direction = convert(I, 1)
            j_ray_direction = convert(I, 0)
            i_ray_stop_tile, j_ray_stop_tile, hit_dimension, signed_perpendicular_distance_to_obstacle = RC.cast_ray(obstacle_tile_map, cells_per_tile_along_axis, i_ray_start_cell, j_ray_start_cell, i_ray_direction, j_ray_direction, max_steps)
